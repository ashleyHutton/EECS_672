...
void ModelView::addToGlobalPan(double dxInLDS, double dyInLDS, double dzInLDS)
{
	...
	cryph::Matrix4x4 trans = cryph::Matrix4x4::translation(dxInEC, dyInEC, 0);
...
void ModelView::addToGlobalRotationDegrees(double rx, double ry, double rz)
{
	...
	dynamic_view = cryph::Matrix4x4::xRotationDegrees(rx) * cryph::Matrix4x4::yRotationDegrees(ry) * dynamic_view;
}
...
void ModelView::getMatrices(cryph::Matrix4x4& mc_ec, cryph::Matrix4x4& ec_lds)
{
	...
	mc_ec = cryph::Matrix4x4::translation(0, 0, -distEyeCenter) * dynamic_view * cryph::Matrix4x4::translation(0, 0, distEyeCenter) * M_ECu;
	...
	double delta_mcX = ModelView::mcRegionOfInterest[1] - ModelView::mcRegionOfInterest[0];
	double delta_mcY = ModelView::mcRegionOfInterest[3] - ModelView::mcRegionOfInterest[2];
	double delta_mcZ = ModelView::mcRegionOfInterest[5] - ModelView::mcRegionOfInterest[4];

	double maxDelta = 0.0;
	if (delta_mcX > maxDelta) { maxDelta = delta_mcX; }
	if (delta_mcY > maxDelta) { maxDelta = delta_mcY; }
	if (delta_mcZ > maxDelta) { maxDelta = delta_mcZ; }
